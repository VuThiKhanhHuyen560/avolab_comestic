...
export const AdminLoginPage: React.FC = () => {
  ...
  const [email, setEmail] = useState('admin@avolab.demo');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (result.success && result.user?.role === 'ADMIN') {
        ...
        setErrorMsg(`Account exists but is registered as ${result.user?.role}. Administrator credentials required.`);
      } else {
        setPassword('');
  ...
  return (
    <AuthenticationLayout portalType="ADMIN" subtitle="Executive Business Intelligence & System Administration">
      <div className="space-y-5">
        ...
        <div className="text-center space-y-1">
          ...
          <p className="text-xs text-[#707070]">
            Sign in to manage the AVOLAB COSMETICS platform.
          </p>
        </div>
        ...
        <div className="bg-[#F0EBE1] border border-[#E6E1D6] rounded-2xl p-3.5 space-y-2 text-left">
          <div className="flex items-center justify-between">
            <span className="text-[10px] uppercase font-bold tracking-widest text-[#4A5D4E] flex items-center gap-1">
              <Sparkles size={13} className="text-[#849673]" /> Demo Administrator Credentials
            </span>
          ...
          <div className="flex items-center justify-between text-xs bg-white p-2.5 rounded-xl border border-[#E6E1D6]">
            <div>
              <p className="font-mono text-[11px] text-[#2D2D2D] font-semibold">admin@avolab.demo</p>
